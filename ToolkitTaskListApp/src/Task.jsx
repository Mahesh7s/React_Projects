	  ...
	  <div>
		...
		{tasks.length>0 && tasks.map((task,index)=>(
			<div key={index} >
				...
				<span style={{padding:"30px",margin:"20px"}}><b>Title:</b>{task.title}      
				 <b>Status:</b>{task.status?"Completed":"Pending"}      
				 <button onClick={()=>dispatch(toggleTask(task.id))}>{task.status?"Mark as Pending":"Mark as Completed"}</button> 
