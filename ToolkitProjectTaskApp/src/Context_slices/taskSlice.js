...
const baseUrl = "https://snacks-45e95-default-rtdb.asia-southeast1.firebasedatabase.app";

// 👉 Add a task to a project
export const addTaskToProject = createAsyncThunk(
  "tasks/addTaskToProject",
  async ({ projectId, task }) => {
    const res = await axios.post(`${baseUrl}/projects/${projectId}/tasks.json`, task);
...
export const updateTask = createAsyncThunk(
  ...
  async ({ projectId, taskId, updatedTask }) => {
    await axios.patch(`${baseUrl}/projects/${projectId}/tasks/${taskId}.json`, updatedTask);
...
export const { setSearchTerm, setFilter, setSortBy, setPage } = taskSlice.actions;

export default taskSlice.reducer;


export const selectProcessedTasks = (projectTasks, { searchTerm, filter, sortBy, pagination }) => {
  let tasks = Object.entries(projectTasks || {}).map(([id, task]) => ({ ...task, id }));

  if (filter.priority !== "all") {
    tasks = tasks.filter((t) => t.priority === filter.priority);
  }
  if (filter.status !== "all") {
    tasks = tasks.filter((t) => t.completed === (filter.status === "completed"));
  }
