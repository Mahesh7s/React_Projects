...
import {
  fetchProjectById,
} from "../Context_slices/projectSlice";
...
export default function SingleProject() {
  ...
  // Rename selected to project
  let { selected: project, loading, error } = useSelector((state) => state.project);

  // Get tasks slice filters and state
  const taskFilters = useSelector((state) => state.tasks);

  // Safely get processedTasks only if project exists
  const processedTasks = project ? selectProcessedTasks(project.tasks, taskFilters) : [];

  // Task form state with completed boolean, default priority low
  ...
  // Handle input changes, converting strings to booleans for completed select
  ...
  function handleSubmit(e) {
    ...
    dispatch(addTaskToProject({ projectId: id, task })).then(() => {
      dispatch(fetchProjectById(id));
      setTask({ title: "", description: "", completed: false, priority: "low", dueDate: "" });
    });
  }
